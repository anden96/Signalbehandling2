...
Wn=[(1209-10)/(4*10^3) (1209+10)/(4*10^3)];
  Rp=0.5;
  n=2;
  [b a] = cheby1(n,Rp,Wn);
    y=filter(b,a,x);
...
%     med=mean(abs(fft(x)))
%     maximumx=max(abs(fft(x1)))
%     maximumy=max(abs(fft(y)))
xmean=mean(abs(x)).^2
  ymean=mean(abs(y)).^2
  ymean/xmean
%%
...
disp(" ");
x1 = DTMFgen('AB30',40,40,2);
for iterator=1:4
% 
    x= x1((iterator-1)*640+320:(iterator)*640,:);
...
%  C2=960+(iterator-1)*640
    ...
    for k=1:4
        ...
        % Kommer behövas justeras när fler tecken ska in. Skapa en speciell vektor att studera
    ...
    for k=5:8
        ...
        fs=8000;
         ...
         value = mean(abs(y)).^2 ; % Kommer behövas justeras när fler tecken ska in. Skapa en speciell vektor att studera
